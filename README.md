...
- H2 para pruebas automatizadas y demostración local del catálogo precargado.
- Maven.
...
El archivo oficial es [piscinas_script.sql](piscinas_script.sql). **El script elimina y recrea las tablas**, por lo que debe ejecutarse solamente en una base de desarrollo o demostración que pueda reemplazarse.
...
### Demostración inmediata con productos precargados

Para ejecutar la versión oficial sin configurar primero la contraseña local de MySQL:

```powershell
mvn spring-boot:run "-Dspring-boot.run.profiles=local"
```

El perfil `local` utiliza H2 en memoria y carga automáticamente doce productos, cuatro categorías, imágenes del catálogo y las dos cuentas de prueba. Los datos se reinician al detener la aplicación. La entrega conserva MySQL como base permanente mediante las variables indicadas anteriormente.

La guía ampliada se encuentra en [docs/configuracion-mysql.md](docs/configuracion-mysql.md). El archivo `.env.example` es solo una referencia; Spring Boot no carga `.env` automáticamente.
